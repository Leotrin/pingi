<nav class="navbar navbar-fixed-top">
    <div class="container">
        <div class="navbar-brand">
            <a href="{{url('/home')}}"><img src="/assets/images/logo.png" alt="Pingi Logo" class="img-responsive logo" style="width:30px;margin-top:0;">
                <span style="color:#707070;">pingi.io</span>
            </a>
        </div>

        <div class="navbar-right">
            <form id="navbar-search" method="post" action="/website/search" class="navbar-form search-form">
                @csrf
                <input value="@if(isset($s)){{$s}}@endif" name="search" class="form-control" placeholder="Search here..." type="text">
                <button type="submit" class="btn btn-default"><i class="icon-magnifier"></i></button>
            </form>

            <div id="navbar-menu">
                <ul class="nav navbar-nav">
                    <li>
                        <a href="{!! url("/profile") !!}" class="icon-menu d-none d-sm-block rightbar_btn"><i class="icon-user"></i></a>
                    </li>
                    <li>
                        <form action="{!! route("logout") !!}" method="POST">
                            @csrf
                            <a class="icon-menu d-none d-sm-none d-md-none d-lg-block">
                                <button type="submit" style="border:none;background-color: transparent;"><i class="icon-login"></i></button></a>
                        </form>
                       </li>
                </ul>
            </div>
        </div>

        <div class="navbar-btn">
            <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbar" aria-expanded="false">
                <i class="lnr lnr-menu fa fa-bars"></i>
            </button>
        </div>
    </div>
</nav>
